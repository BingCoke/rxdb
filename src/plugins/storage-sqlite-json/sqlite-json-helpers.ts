...
export function mangoQueryToSQLiteJSON(
  ...
  switch (operator) {
    ...
    case '$or':
      ...
      const orConditions = value.map((condition, idx) => {
        ...
        const subField = keys[0];
        const subOperator = Object.keys(condition[subField])[0];
        const subValue = condition[subField][subOperator];
